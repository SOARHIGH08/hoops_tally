...
app.post("/teams/checkTeamName", (req, res) => {
  ...
  const query_team = `SELECT * FROM teams WHERE team_name = ?`;

  db.query(query_team, [teamName], (err, results) => {
    if (err) {
      console.error(err);
      res.status(500).json({ message: "Server error" });
      return;
    ...
    if (results.length === 1) {
      res.json({
        message: results.length + " team name found",
        isUserExist: results.length,
      });
    } else {
      res.json({
...
app.post("/teams/addTeam", (req, res) => {
  ...
  const sqlInsertTeam =
    "INSERT INTO teams (team_name, organizer_id) VALUES (?, ?)";

  db.query(sqlInsertTeam, [teamName, organizer_id], (err, result) => {
    if (err) {
      console.log("error");
    } else {
...
app.get("/games-list", (req, res) => {
  const { organizer_id, game_status } = req.query;
  const query = `
      SELECT g.*, 
...
app.get("/players", (req, res) => {
  ...
  const query = `
      ...
      WHERE team = ? OR team = ?
      ORDER BY CASE WHEN team = ? THEN 1 ELSE 2 END
    `;

  db.query(query, [teamA_id, teamB_id, teamA_id], (err, results) => {
    if (err) {
